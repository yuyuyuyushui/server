...
def file_execute(sql,**kwargs):
    conn_params = settings.DATABASE**kwargs
    db_path = '%s/%s' % (conn_params['path'], conn_params['name'])

    print(sql,db_path)
    sql_list = sql.split("where")#update accounts where account=%s" % account_data['id'],account_data=account_data
    print(sql_list)
    if sql_list[0].startswith("select") and len(sql_list) > 1:#startswith判断字符串是否以指定字符或子字符串开头
        ...
        column, val = sql_list[1].strip().split("=")#account=%s account_data=account_data
        if column == 'account':
            account_file = "%s/%s.json" % (db_path, val)
            #print(account_file)
            if os.path.isfile(account_file):
